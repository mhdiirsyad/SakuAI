import { CATEGORY_KEYS_EXPENSE, CATEGORY_KEYS_INCOME, CategoryKey } from "@/constants/category";
...
export async function callGemini(promptText: string, inlineData: { mimeType: string, data: string }) {
...
export async function extractFromReciept(
    base64Image: string,
    mimeType: string
...
export async function extractFromVoice(
    ...
    const prompt = `You are transcribing a short voice note for a personal finance app where the user is logging a transaction (e.g. "I spent 400 on groceries yesterday" or "Got 5000000 rupiahs freelance payment today"). Today's date is ${today}. Extract the transaction details.
